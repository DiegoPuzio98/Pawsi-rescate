...
create policy if not exists "Users can delete their blocks"
on public.user_blocks
for delete
using (auth.uid() = blocker_id);

-- Allow participants to delete their messages (to enable deleting conversations)
-- RLS is already enabled on messages; add delete policy
create policy if not exists "Participants can delete messages"
on public.messages
for delete
